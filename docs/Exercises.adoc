...
[#eql-primer]**EQL Primer**: An EQL query includes 1) _properties_ such as `:list/label`, 2) _joins_ of the form `{<property or ident> <query>}`, 3) _idents_ such as `[:item/id 1]` to ask for the data of the entity with that ident (and we can again use a join to precise what data).
...
2. Click on the transaction (i.e. mutation) to see its details. Notice that the transaction is simply data, of the form `(<fully qualified symbol> {options...})`. Let's explore the details of its query. +
...
4. Now click the _[Send to query]_ button next to the Response in the details, which will send you to the _EQL_ tab. Once there, press also the _[(Re)load Pathom Index]_ button to the very right (for autocompletion) and then the _[Run EQL]_ button next to it. Here we can play with EQL queries that the backend can answer.
...
7. Change the query to the ident query `[[:item/id 1]]`. What do you get back?
...
1. Go to `ui.cljs` and evaluate the `df/load!` form marked _Exercise 7.1_ +
   (Note: df/load! is just a helper function that eventually transacts a mutation)
2. Notice that it only returns a uuid of the submitted transaction, which is executed asynchronously
3. Also notice that we pass it the ident of the thing we want to load and a component - internally, Fulcro will `(comp/get-query <the component>)`, and this is the query we can see sent to the backend.
...
5. Go to _Fulcro Inspect - DB_ and expand `:list/id` to verify the list `2` has been added there. Then expand `:item/id` to verify that the item `99` has also been added there.
6. Notice that Fulcro has placed each _data entity_ - both list and item - into their correct "tables" (`:list/id` and `:item/id`, respectively) and that the list links to the item using its ident. I.e. Fulcro has normalized the incoming data tree into the Client DB's "entity tables". That is only possible because both `TodoList` and `TodoItem` declare their `:ident` and thanks to the metadata linking each (sub)query to the component it comes from:
+
```clojure
(defsc TodoItem [_ _]
...
(defsc TodoList [_ _]
  ...
  ...)
```
7. At the top left of the _DB_ tab, click on the `<` next to the slider a few times, until the list 2 and item 99 disappear from the DB. This way you can go back and forward in the history (and the UI, since it will always reflect the DB state - though there is nothing to see now).

LESSON: Load targets a backend resolver by providing the inputs it responds to. It includes a component's query to request a particular subset of the available data and its (and children's) ident to normalize the data into the Client DB. You can use the DB view to go back in history and look at the data and UI at any prior point.

### 8. Changing local and remote data with mutations

To change state either in the frontend or in the backend, you "transact" (submit to the transaction subsystem) a mutation. A mutation is just data, namely the list `(<namespaced symbol> {<parameter key-value pairs>})`. You can register a "handler" for that symbol both on the frontend and backend using the Fulcro/Pathom `defmutation` macros. Let's trigger a mutation and explore it.

1. Mark "Item 3" as complete by clicking to the left of it. This essentially triggers this DOM event handler (simplified):
+
```clojure
:onChange #(comp/transact! this [(api/todo-check {:id (:item/id props)})])
```
+
Note: The `todo-check` call simply returns a list of its name and parameters, as you will soon see
